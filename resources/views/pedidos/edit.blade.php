            ...
            <div class="modal-body">
                <form class="g-3 needs-validation" method="POST" action="{{ route('pedidos.update',['pedido' => $pedido->id]) }}" novalidate>
                      ...
                      <div class="col-9">
                        <select id="id_produto" name="id_produto" class="form-control @error('id_produto') is-invalid @enderror" required>
                          @forelse($Produtos as $produto)
                            <option value="{{ $produto->id }}"  @if($produto->id == $pedido->itens_pedido[0]->produto->id) selected @endif>
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-9">
                        <input type="number" id="quantidade" name="quantidade" class="form-control @error('quantidade') is-invalid @enderror"  value="{{$pedido->itens_pedido[0]->quantidade}}" required>
                        @error('quantidade')
                          <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                      </div>
